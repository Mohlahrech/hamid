...
class AccountInvoice(models.Model):
    ...
    timbre_arrondi = fields.Monetary(
        string='Timbre Arrondi',
        compute='_compute_timbre_arrondi',
        store=True,
        readonly=True
    )

    @api.depends('amount_timbre')
    def _compute_timbre_arrondi(self):
        for record in self:
            record.timbre_arrondi = round(record.amount_timbre)
            ...
            if record.payment_mode.mode_type == 'cash':
                if not record.company_id.sale_timbre:
                    raise ValidationError(
                        _('Verifier si les comptes contrepartie sont rempli sur la configuration de timbre'))
    ...
    #         other_moves._post(soft=False)
        ...
        # # Trigger the recomputation of amount_timbre and timbre lines
        # for record in self:
        #     record._compute_amount_timbre()
        #
        # return False
